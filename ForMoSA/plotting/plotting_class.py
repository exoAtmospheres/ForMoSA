...
class PlottingForMoSA():
    ...
    Author: Paulina Palma-Bifani, Simon Petrus, Matthieu Ravet and Allan Denis, (+ Nathan Zimniak)
    '''
    ...
    def _get_posteriors(self, burn_in=0):
        ...
        Args:
            burn_in         (int): burn-in of the convergence chain.
        Returns:
            None
        '''
        ...
        if len(self.global_params.log_amp) > 3: # If you want separate log_amp for each observations
            for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
                if self.global_params.log_amp[indobs*3] != 'NA' and self.global_params.log_amp[indobs*3] != 'constant': # Check if the idobs is different from constant
                    tot_list_param_title.append(extra_parameters[7][1] + fr'$_{indobs}$' + ' ' + extra_parameters[7][2])
                    theta_index.append(f'log_amp_{indobs}')
        else: # If you want 1 common log_amp for all observations
            if self.global_params.log_amp[0] != 'NA' and self.global_params.log_amp[0] != 'constant':
                tot_list_param_title.append(extra_parameters[7][1] + ' ' + extra_parameters[7][2])
                theta_index.append('log_amp')
        if len(self.global_params.log_len) > 3: # If you want separate log_len for each observations
            for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
                if self.global_params.log_len[indobs*3] != 'NA' and self.global_params.log_len[indobs*3] != 'constant': # Check if the idobs is different from constant
                    tot_list_param_title.append(extra_parameters[8][1] + fr'$_{indobs}$' + ' ' + extra_parameters[8][2])
                    theta_index.append(f'log_len_{indobs}')
        else: # If you want 1 common log_len for all observations
            if self.global_params.log_len[0] != 'NA' and self.global_params.log_len[0] != 'constant':
        ...
        if self.global_params.bb_t[0] != 'NA' and self.global_params.bb_t[0] != 'constant':
            tot_list_param_title.append(extra_parameters[9][1] + ' ' + extra_parameters[9][2])
            theta_index.append('bb_t')
        if self.global_params.bb_r[0] != 'NA' and self.global_params.bb_r[0] != 'constant':
            tot_list_param_title.append(extra_parameters[10][1] + ' ' + extra_parameters[10][2])
        ...
        if self.global_params.r[0] != 'NA' and self.global_params.r[0] != 'constant':
            ...
            for res, results in enumerate(self.samples):
                ...
                lum = 4 * np.pi * (r_picked * cst.R_jup.value) ** 2 * results[0] ** 4 * cst.sigma_sb.value

                # Put in log and normalize with respect to the solar value
                log_lum = np.log10(lum / cst.L_sun.value)

                results = np.concatenate((results, np.asarray(log_lum)))
    ...
    def _get_outputs(self, theta):
        ...
        for indobs, obs in enumerate(sorted(glob.glob(self.global_params.main_observation_path))):
            ...
            obs_dict = dict(np.load(os.path.join(self.global_params.result_path, f'spectrum_obs_{obs_name}.npz'), allow_pickle=True))
            ...
            # Prepare covariance padding and determinant (if necessary)
            if len(obs_dict['cov']) != 0:
                obs_dict['log_det_cov'] = np.log(np.linalg.det(obs_dict['cov']))
                if self.global_params.log_amp[indobs*3 % len(self.global_params.log_amp)] != "NA" and self.global_params.log_len[indobs*3 % len(self.global_params.log_len)] != "NA":
    ...
    def _get_model_spectrum(self, theta, grid_used='original', wav_bounds=[], res=None, re_interp=False, compute_scale=True, indobs=0):
        '''
        ...
        Args:
            ...
            res                          (int): (default = None) Spectral resolution (at Nyquist). If None, the original resolution of the model will be used.
            re_interp                (boolean): (default = False). Option to reinterpolate or not the grid.
            compute_scale            (boolean): (default = False). Option to compute the scaling factor or not.
        ...
        if res is None:
            wav_mod = wav_mod_nativ
            flx_mod = flx_mod_nativ
            res_mod = res_mod_nativ
        else:
    ...
    def _get_PT_chem(self, theta, path_PT_chem_grid, grid_used='original'):
        '''
        ...
        Args:
            theta                       (list): best parameter values
            path_PT_chem_grid            (str): path to the PT grid in xarray format
            grid_used                    (str): (default = 'original') Path to the grid from where to extract the spectrum. If 'original', the current grid will be used.
        Returns:
