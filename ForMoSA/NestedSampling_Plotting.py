...
class NestedSampling_Plotting(object):
    ...
    def _plot_data_point(ax: matplotlib.axes.Axes, axr: matplotlib.axes.Axes, obs_wav: np.ndarray, obs_flx: np.ndarray, mod_flx: np.ndarray, std_global: float, color: str, edgecolor: str, marker: str, size, label: str=None, yerr: np.ndarray=None, xerr: np.ndarray=None, plot_model: str='plot') -> None:
        ...
        if marker == 'NA':
            ...
            sc = ax.scatter(obs_wav, obs_flx, c=color, edgecolors=edgecolor, marker=marker, s=size, label=label, linewidths=2)
        ...
        return sc  # Return scatter object for legend handling
    ...
    def _get_plot_style(self, indobs: int, default_color: str='magenta', default_edge: str='darkmagenta', default_size: float=50, default_marker: str='o') -> tuple[str, str, str, float]:
        '''
