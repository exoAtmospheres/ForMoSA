import numpy as np
import xarray as xr

# ----------------------------------------------------------------------------------------------------------------------


def yesno(text):
    '''
    Function to interact with the terminal and decide for different options when running ForMoSA (Loop to repeat question if answer is different to 'y' or 'n).

    Args:
        text    (str): (y/n) answer in the terminall in interactive mode
    Returns:
        asw     (str): answer y or n

    Author: Simon Petrus
    '''
    print(text)
    asw = input()
    if asw in ['y', 'n']:
        return asw
    else:
        return yesno()

# ----------------------------------------------------------------------------------------------------------------------


def decoupe(second):
    """
    Re-arranged a number of seconds in the hours-minutes-seconds format.

    Args:
        second (float): number of second
    Returns:
        - float     : hours
        - float     : minutes
        - float     : seconds

    Author: Simon Petrus
    """

    hour = second / 3600
    second %= 3600
    minute = second / 60
    second %= 60

    return hour, minute, second


# ----------------------------------------------------------------------------------------------------------------------


def find_nearest(array, value):
    """
    Return the indice of the closest values from a desire value in an array.

    Args:
        array (array): Array to explore
        value (float): Desire value
    Returns:
        - idx (int)          : Indice of the closest values from the desire value

    Author: Simon Petrus
    """
    idx = (np.abs(array - value)).argmin()

    return idx


# ----------------------------------------------------------------------------------------------------------------------


def format_grid(grid, attr, free_comp, weights):
    """
    Format PCA or NMF outputs into a single xarray
    
    Args:
        - grid              (np.ndarray): Original grid 
        - attr                    (dict): Original grid attributs
        - free_comp                (int): Number of free components in the new grid (= PCA component used during PCA + 1 (nfs))
        - weights           (np.ndarray): PCA or NMF weights grid
    Returns:
        ds_weights              (xarray): Xarray of the PCA or NMF weights grid

    Author: Matthieu Ravet
    """
    # Format the new grids in xarray
    vars_nfs_ws = ["eigen_indices"]
    for key in attr['key']:
        vars_nfs_ws.append(key)
    coords_nfs_ws = {"eigen_indices": np.arange(free_comp)} # The first columns are the normalization factors so you need to add 1
    for key in attr['key']:
        coords_nfs_ws[key] = grid[key].values
    ds_weights= xr.Dataset(data_vars=dict(grid=(vars_nfs_ws, weights)), coords=coords_nfs_ws, attrs=attr)

    return ds_weights


# ----------------------------------------------------------------------------------------------------------------------



def pad_fct(wav, cov, pad_value=0, max_wav_sep=np.inf):
    """
    Pads a covariance matrix and corresponding wavelength separation into banded form.

    Args:
        wav                  (np.ndarray): Wavelength grid (not necessarily uniform).
        cov                  (np.ndarray): Full covariance matrix (N x N).
        pad_value                 (float): Value used to pad unused entries in the banded matrix.
        max_wav_sep     (float, optional): Cutoff wavelength separation. If None, set to max wavelength step.

    Returns:
        tuple: (banded_wav_sep, banded_cov) as np.ndarrays suitable for banded operations.

    Author : Matthieu Ravet adapt from De Regt et al. (2025)
    """
    wav_sep = np.abs(wav[None, :] - wav[:, None])
    if max_wav_sep is None:
        max_wav_sep = np.max(np.diff(wav))

    banded_wav_sep = []
    banded_cov = []

    for k in range(wav_sep.shape[0]):
        wav_diag = np.diag(wav_sep, k=k)
        cov_diag = np.diag(cov, k=k)

        wav_diag = np.pad(wav_diag, (0, k), constant_values=pad_value) # Large values to make sure they are masked
        cov_diag = np.pad(cov_diag, (0, k), constant_values=0) # Will be replace anyway during the inversion

        if (wav_diag == 0).all() and k != 0:
            break
        if (wav_diag > max_wav_sep).all():
            break

        banded_wav_sep.append(wav_diag)
        banded_cov.append(cov_diag)

    return np.asarray(banded_wav_sep), np.asarray(banded_cov)

