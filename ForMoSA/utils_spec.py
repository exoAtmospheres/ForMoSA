...
def convolve_and_sample(wv_channels, sigmas_wvs, model_wvs, model_fluxes, num_sigma=3, force_int=True): # num_sigma = 3 is a good compromise between sampling enough the gaussian and fast interpolation
    ...
    filter_size = int(np.ceil(np.max((2 * num_sigma * sigmas_wvs) / np.median(dwv_model)))) # !!!! replacing min by median to avoid too large filter size in case of some outliers in the wavelength grid of the model, but need to check that it does not cause problems in some cases.
...
def GP_cov_fct(wav, pad_sep_wav, pad_cov, log_amp, log_len, trunc_dist=3):
    '''
    Function to parametrize the covariance matrix using GP

    Args:
        wav             (array): Wavelength grid (spectroscopy)
        pad_sep_wav   (n-array): Padded wavelength separation map (in µm)
        pad_cov       (n-array): Padded covariance (in (W/m²/µm)²)
        log_amp         (float): Log-correlation amplitude (in log(W/m²/µm))
        log_len         (float): Log-correlation length (in log(µm)
        trunc_dist        (int): Truncation distance in number of correlation length away from the diagonal
    Returns:
        - inv_cov     (n-array): Inverse covariance matrix

    Author: Matthieu Ravet adapted from De Regt et al. 2025
    '''
    # Size of the problem
    N = len(wav)
    ...
    try:
        # Wave-mask
        w_ij = (pad_sep_wav < trunc_dist*l)

        # Gaussian radial-basis function kernel
        pad_cov[w_ij] += a**2 * np.exp(-0.5 * (pad_sep_wav[w_ij])**2 / l**2)

        # if local_kernels is not None:
        #     mu = 1.6
        #     a_loc = a
        #     l_loc = 2*l

        #     for diag_idx in range(wav.shape[0]):
        #         offset = diag_idx
        #         N = wav.shape[0] - offset
        #         i = np.arange(N)        # index in row
        #         j = i + offset          # index in column
        #         # Now this is safe — diagonal aligned
        #         mu_i = wav[i] - mu
        #         mu_j = wav[j] - mu
        #         # Compute squared separation
        #         sq_dist = mu_i**2 + mu_j**2
        #         # Mask: only include close-enough pairs
        #         mask = sq_dist < (trunc_dist * l_loc) ** 2

        #         # skip if nothing passes the mask
        #         if not np.any(mask):
        #             break 

        #         local_val = np.zeros(N)
        #         local_val[mask] = a_loc**2 * np.exp(-0.5 * sq_dist[mask] / l_loc**2)
        #         pad_cov[diag_idx, :N] += local_val


        # Compute banded Cholesky decomposition on non-zero diagonals
        mask_nonzero_diag = (pad_cov != 0).any(axis=1)

        pad_cov = pad_cov[mask_nonzero_diag,:]
        pad_cov_cholesky = cholesky_banded(pad_cov, lower=True, check_finite=False)

        # Log determinant of the covariance matrix
        logdet = 2*np.sum(np.log(pad_cov_cholesky[0]))

        # Invert using banded-cholesky decomposition Ax = b
        b = np.eye(N)
        inv_cov = cho_solve_banded((pad_cov_cholesky, True), b, check_finite=False)

    except (np.linalg.LinAlgError, ValueError, RuntimeError):
        # Cholesky failed (not positive definite)
        inv_cov, logdet = None, None  # or return an "invalid" marker to your likelihood function
