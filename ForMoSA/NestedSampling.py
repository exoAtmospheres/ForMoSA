...
class NestedSampling(object):
    ...
    def run(self, logL_type: list, results_path: str | os.PathLike, observation: Observation, modelgrid: ModelGrid, interp_method: str = 'linear', wav_cont: list = ['NA'], res_cont: list = ['NA'], hc_type: list = ['NA'], bounds_lsq: list = ['NA'], emulator: list = ['NA']) -> None:
        ...
        for indobs in range(observation.n_obs):
            ...
            if len(observation.obs_data[indobs]['spectro']['inv_cov']) > 0 and not(logL_type[indobs].endswith('_covariance')):
        ...
        loglike_gp = lambda theta: self._loglike(theta, observation, modelgrid, interp_method=interp_method, wav_cont=wav_cont, res_cont=res_cont, bounds_lsq=bounds_lsq, emulator=emulator, hc_type = hc_type)
    ...
    def _loglike(self, theta: list, observation: Observation, modelgrid: ModelGrid, wav_cont: list = ['NA'], res_cont: list = ['NA'], bounds_lsq: list = ['NA'], interp_method: str = 'linear', emulator: list = ['NA'], hc_type: list = ['NA']) -> float | tuple[dict, np.ndarray, np.ndarray]:
        '''
    ...
    def _compute_model_from_theta(self, theta: list, obs_dict_spectro: dict, obs_dict_photo, grid_spectro: ModelGrid | ModelSubGrid, grid_photo: ModelGrid | ModelSubGrid, interp_method: str = 'linear', wav_cont: str | np.ndarray = 'NA', res_cont: str | np.ndarray = 'NA', bounds_lsq: list = ['NA', 'NA'], hc_type: str = 'NA', indobs: int = 0) -> tuple[dict, dict]:
        '''
        ...
        wav_mod_spectro, res_mod_obs_spectro = grid_spectro.wavelength, grid_spectro.resolution
        ...
        flx_mod_spectro_nativ = flx_mod_spectro_nativ * ck_spectro
