...
def extract_observation(global_params, indobs=0):
    ...
    with fits.open(global_params.observation_path) as hdul:
        ...
        err_photo, err_spectro = err[mask_photo], err[~mask_photo]
        if len(cov) != 0:
...
def adapt_model(global_params, obs_dict, wav_mod_nativ, flx_mod_nativ, res_mod_nativ, target_wav_mod, target_res_mod, indobs=0):
    ...
    if len(obs_dict['wav_spectro']) != 0:
        ...
        # If we want to estimate and substract the continuum of the model (except for high contrast where we need to keeo the og spectrum):
        if global_params.res_cont[indobs % len(global_params.res_cont)] != 'NA' and len(obs_dict['star_flx']) == 0:
