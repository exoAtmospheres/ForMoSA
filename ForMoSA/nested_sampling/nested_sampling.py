...
from ForMoSA.utils import pad_fct
    ...
    for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
        ...
        for key in obs_dict:
            if obs_dict[key].size != 0:
                ...
                elif key != 'cov' and key != 'inv_cov': # Only cut 1D arrays
        ...
        # Prepare covariance padding and determinant and interpolating model resolution onto the data (if necessary)
        if len(obs_dict['wav_spectro']) != 0:
            ...
            if len(obs_dict['cov']) != 0:
                sign, logdet = np.linalg.slogdet(obs_dict['cov'])
                if sign <= 0:
                    raise ValueError("Covariance matrix is not positive definite.")
                obs_dict['log_det_spectro'] = logdet
                if global_params.log_amp[indobs*3 % len(global_params.log_amp)] != "NA" and global_params.log_len[indobs*3 % len(global_params.log_len)] != "NA":
                    obs_dict['pad_sep_wav_spectro'], obs_dict['pad_cov'] = pad_fct(obs_dict['wav_spectro'], obs_dict['cov'], pad_value=1000)
                else:
                    pass
            else:
...
def loglike(theta, theta_index, global_params, main_file, for_plot='no'):
    ...
    for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
        ...
        if len(obs_dict_modif['wav_photo']) != 0:
            if global_params.logL_type[indobs % len(global_params.logL_type)] == 'chi2_noisescaling':
                logL_photo = logL_chi2_noisescaling(obs_dict_modif['flx_photo']-flx_mod_photo_modif, obs_dict_modif['err_photo'], obs_dict_modif['log_det_photo'], full=logL_full)
            else:
                logL_photo = logL_chi2(obs_dict_modif['flx_photo']-flx_mod_photo_modif, obs_dict_modif['err_photo'], obs_dict_modif['log_det_photo'], full=logL_full)
        else:
            logL_photo = 0

        # Computation of the spectroscopy logL
        # If you need to use the full logL computation (for now, its necessary only if you're fitting for the noise)
        if global_params.log_amp[indobs*3 % len(global_params.log_amp)] != "NA" and global_params.log_len[indobs*3 % len(global_params.log_len)] != "NA":
            logL_full = True
        else:
            pass

        # Failsafe in case of ill-conditioned covariance matrix (using the log-determinant as pointer)
        if len(obs_dict_modif['wav_spectro']) != 0:
            # Failsafe in case of ill-conditioned covariance matrix (using the log-determinant as pointer)
            if obs_dict_modif['log_det_spectro'] is None:
                logL_spectro = -1e30
            else:
                if global_params.logL_type[indobs % len(global_params.logL_type)] == 'chi2':
                    logL_spectro = logL_chi2(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['err_spectro'], obs_dict_modif['log_det_spectro'], full=logL_full)
                elif global_params.logL_type[indobs % len(global_params.logL_type)] == 'chi2_covariance' and len(obs_dict_modif['inv_cov']) != 0:
                    logL_spectro = logL_chi2_covariance(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['inv_cov'], obs_dict_modif['log_det_spectro'], full=logL_full)
                elif global_params.logL_type[indobs % len(global_params.logL_type)] == 'chi2_noisescaling':
                    logL_spectro = logL_chi2_noisescaling(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['err_spectro'], obs_dict_modif['log_det_spectro'], full=logL_full)
                    ...
                    logL_spectro = logL_chi2_noisescaling_covariance(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['inv_cov'], obs_dict_modif['log_det_spectro'], full=logL_full)
...
def prior_transform(theta, theta_index, lim_param_grid, global_params):
    ...
    if len(global_params.ld) > 3: # If you want separate ld for each observations
        ...
        if prior_law_ld != 'NA' and prior_law_ld != 'constant':
            ...
            prior.append(prior_ld)
    if len(global_params.log_amp) > 3: # If you want separate log_amp for each observations
        for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
            prior_law_log_amp = global_params.log_amp[indobs*3] # Prior laws should be separeted by 2 values (need to be upgraded)
            if prior_law_log_amp != 'NA' and prior_law_log_amp != 'constant':
                ind_theta_log_amp = np.where(theta_index == f'log_amp_{indobs}')
                if prior_law_log_amp == 'uniform':
                    prior_log_amp = uniform_prior([float(global_params.log_amp[indobs*3+1]), float(global_params.log_amp[indobs*3+2])], theta[ind_theta_log_amp[0][0]]) # Prior values should be by two
                if prior_law_log_amp == 'loguniform':
                    prior_log_amp = loguniform_prior([float(global_params.log_amp[indobs*3+1]), float(global_params.log_amp[indobs*3+2])], theta[ind_theta_log_amp[0][0]])
                if prior_law_log_amp == 'gaussian':
                    prior_log_amp = gaussian_prior([float(global_params.log_amp[indobs*3+1]), float(global_params.log_amp[indobs*3+2])], theta[ind_theta_log_amp[0][0]])
                prior.append(prior_log_amp)
    else: # If you want 1 common corr_len for all observations
        prior_law_log_amp = global_params.log_amp[0]
        if prior_law_log_amp != 'NA' and prior_law_log_amp != 'constant':
            ind_theta_log_amp = np.where(theta_index == 'log_amp')
            if prior_law_log_amp == 'uniform':
                prior_log_amp = uniform_prior([float(global_params.log_amp[1]), float(global_params.log_amp[2])], theta[ind_theta_log_amp[0][0]])
            if prior_law_log_amp == 'loguniform':
                prior_log_amp = loguniform_prior([float(global_params.log_amp[1]), float(global_params.log_amp[2])], theta[ind_theta_log_amp[0][0]])
            if prior_law_log_amp == 'gaussian':
                prior_log_amp = gaussian_prior([float(global_params.log_amp[1]), float(global_params.log_amp[2])], theta[ind_theta_log_amp[0][0]])
            prior.append(prior_log_amp)
    if len(global_params.log_len) > 3: # If you want separate corr_len for each observations
        for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
            prior_law_log_len = global_params.log_len[indobs*3] # Prior laws should be separeted by 2 values (need to be upgraded)
            if prior_law_log_len != 'NA' and prior_law_log_len != 'constant':
                ind_theta_log_len = np.where(theta_index == f'log_len_{indobs}')
                if prior_law_log_len == 'uniform':
                    prior_log_len = uniform_prior([float(global_params.log_len[indobs*3+1]), float(global_params.log_len[indobs*3+2])], theta[ind_theta_log_len[0][0]]) # Prior values should be by two
                if prior_law_log_len == 'loguniform':
                    prior_log_len = loguniform_prior([float(global_params.log_len[indobs*3+1]), float(global_params.log_len[indobs*3+2])], theta[ind_theta_log_len[0][0]])
                if prior_law_log_len == 'gaussian':
                    prior_log_len = gaussian_prior([float(global_params.log_len[indobs*3+1]), float(global_params.log_len[indobs*3+2])], theta[ind_theta_log_len[0][0]])
                prior.append(prior_log_len)
    else: # If you want 1 common corr_len for all observations
        prior_law_log_len = global_params.log_len[0]
        if prior_law_log_len != 'NA' and prior_law_log_len != 'constant':
            if prior_law_log_len != 'constant':
                ind_theta_log_len = np.where(theta_index == 'log_len')
                if prior_law_log_len == 'uniform':
                    prior_log_len = uniform_prior([float(global_params.log_len[1]), float(global_params.log_len[2])], theta[ind_theta_log_len[0][0]])
                if prior_law_log_len == 'loguniform':
                    prior_log_len = loguniform_prior([float(global_params.log_len[1]), float(global_params.log_len[2])], theta[ind_theta_log_len[0][0]])
                if prior_law_log_len == 'gaussian':
                    prior_log_len = gaussian_prior([float(global_params.log_len[1]), float(global_params.log_len[2])], theta[ind_theta_log_len[0][0]])
                prior.append(prior_log_len)
...
def launch_nested_sampling(global_params):
    ...
    if len(global_params.log_amp) > 3:
        for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
            if global_params.log_amp[indobs*3] != 'NA' and global_params.log_amp[indobs*3] != 'constant': # Check if the idobs is different from constant
                n_free_parameters += 1
                theta_index.append(f'log_amp_{indobs}')
    else:
        if global_params.log_amp[0] != 'NA' and global_params.log_amp[0] != 'constant':
            n_free_parameters += 1
            theta_index.append('log_amp')
    if len(global_params.log_len) > 3:
        for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
            if global_params.log_len[indobs*3] != 'NA' and global_params.log_len[indobs*3] != 'constant': # Check if the idobs is different from constant
                n_free_parameters += 1
                theta_index.append(f'log_len_{indobs}')
    else:
        if global_params.log_len[0] != 'NA' and global_params.log_len[0] != 'constant':
            n_free_parameters += 1
            theta_index.append('log_len')
