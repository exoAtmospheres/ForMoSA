...
def modif_spec(global_params, theta, theta_index,
    ...
    if len(global_params.vsini) > 4 and len(global_params.ld) > 3: # If you want separate vsini/ld for each observations
        if global_params.vsini[indobs*4] != "NA" and global_params.ld[indobs*3] != "NA":
            ...
            raise Exception(f'WARNING: You need to define a v.sin(i) AND a limb darkening, or set them both to "NA" for observation {indobs}')
        ...
        if global_params.vsini[0] != "NA" and global_params.ld[0] != "NA":
            ...
            raise Exception('WARNING: You need to define a v.sin(i) AND a limb darkening, or set them both to "NA"')
    ...
    # GP covariance model (cannot be used if you're covariance matrix is already defines)
    if len(global_params.log_amp) > 3 and len(global_params.log_len) > 3: # If you want separate covariance matrix for each observations
        if global_params.log_amp[indobs*3] != "NA" and global_params.log_len[indobs*3] != "NA":
            if global_params.log_amp[indobs*3] == 'constant':
                log_amp_picked = float(global_params.log_amp[indobs*3+1])
            else:
                ind_theta_log_amp = np.where(theta_index == f'log_amp_{indobs}')
                log_amp_picked = theta[ind_theta_log_amp[0][0]]

            if global_params.log_len[indobs*3] == 'constant':
                log_len_picked = float(global_params.log_len[indobs*3+1])
            else:
                ind_theta_log_len = np.where(theta_index == f'log_len_{indobs}')
                log_len_picked = theta[ind_theta_log_len[0][0]]

            # Scale the log_amplitude with the median value of the variance (WARNING : here the inv_cov is modified permenantly !!)
            log_amp_picked += np.log10(np.median(obs_dict['err_spectro']))
            inv_cov_picked, obs_dict['log_det_spectro'] = GP_cov_fct(obs_dict['wav_spectro'], 
                                             obs_dict['pad_sep_wav_spectro'],
                                             np.copy(obs_dict['pad_cov']), 
                                             log_amp_picked, log_len_picked)
            if obs_dict['log_det_spectro'] != None: # Only save the inverse covariance when its well defined
                obs_dict['inv_cov'] = inv_cov_picked
            else:
                pass


        elif global_params.log_amp[indobs*3] == "NA" and global_params.log_len[indobs*3] == "NA":
            pass
        else:
            raise Exception(f'WARNING: You need to define a correlation length AND amplitude for your covariane matrix, or set them both to "NA" for observation {indobs}')

    else: # If you want one covariance matrix
        if global_params.log_amp[0] != "NA" and global_params.log_len[0] != "NA":
            if global_params.log_amp[0] == 'constant':
                log_amp_picked = float(global_params.log_amp[1])
            else:
                ind_theta_log_amp = np.where(theta_index == f'log_amp')
                log_amp_picked = theta[ind_theta_log_amp[0][0]]

            if global_params.log_len[0] == 'constant':
                log_len_picked = float(global_params.log_len[1])
            else:
                ind_theta_log_len = np.where(theta_index == f'log_len')
                log_len_picked = theta[ind_theta_log_len[0][0]]
        ...
        elif global_params.log_amp[0] == "NA" and global_params.log_len[0] == "NA":
            pass
        else:
            raise Exception(f'WARNING: You need to define a correlation length AND amplitude for your covariane matrix, or set them both to "NA"')
    ...
    if len(obs_dict['star_flx']) == 0: # hc already rescale everything
    ...
    if global_params.bb_t[0] != "NA" and global_params.bb_r[0] != "NA":
        if global_params.d[0] != "NA":
            ...
            raise Exception('WARNING: You need to define a distance if you want to fit for the blackbody')        

    elif global_params.bb_t[0] == "NA" and global_params.bb_r[0] == "NA":
        pass

    else:
        raise Exception('WARNING: You need to define a blackbody radius, temperature and a distance, or set them all "NA"')
