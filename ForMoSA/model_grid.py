...
class ModelGrid(object):
    ...
    def _add_subgrid(self, target_wavelength: np.ndarray, target_resolution: np.ndarray):
        ...
        '''
        sub = ModelSubGrid(parent_grid=self, logger=self._logger, target_wavelength=target_wavelength, target_resolution=target_resolution)
...
class ModelSubGrid(ModelGrid):      
    def __init__(self, parent_grid: ModelGrid, logger, target_wavelength: np.ndarray, target_resolution: np.ndarray):
        ...
        self._grid = xr.DataArray(
            data=empty_grid,
            dims=('wavelength',) + parent_grid.grid.dims[1:],
            coords=coords,
            name='grid'
        )
