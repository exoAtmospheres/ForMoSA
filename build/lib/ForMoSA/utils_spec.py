...
def calc_flx_scale(obs_dict, flx_mod_spectro, flx_mod_photo, r_picked, d_picked, alpha=1, mode='physical', use_cov=False):
    ...
    # Calculation of the dilution factor analytically
        ...
        if len(obs_dict['wav_spectro']) != 0:
            if use_cov == True:
                # Compute A = mᵀ Σ⁻¹ m
                A = flx_mod_spectro.T @ obs_dict['inv_cov'] @ flx_mod_spectro
                # Compute b = mᵀ Σ⁻¹ d
                b = flx_mod_spectro.T @ obs_dict['inv_cov'] @ obs_dict['flx_spectro']
                # Solve A x = b
                scale_spectro = np.linalg.solve(A, b)
            else:
...
def GP_cov_fct(wav_spectro, err_spectro, log_amp, log_len):
    '''
    Function to parametrize the covariance matrix using GP

    Args:
        wav_spectro     (array): Wavelength grid (spectroscopy)
        err_spectro     (array): Error bars (spectroscopy)
        N                 (int): Size of the spectroscopic array
        log_amp         (float): Log-correlation amplitude
        log_len         (float): Log-correlation length (in log(microns))
    Returns:
    ...
    import time
    ## GEORGES
    from george import GP as GP_georges
    from george.kernels import ExpSquaredKernel
    # Initialize GP kernel
    t_g = time.perf_counter()
    a = np.exp(log_amp)
    l = np.exp(log_len)
    kernel = a**2 * ExpSquaredKernel(metric=l**2)
    gp = GP_georges(kernel)
    gp.compute(wav_spectro, err_spectro) # error bars are added in quadature to the kernel here

    # Compute inverse
    N = len(wav_spectro)
    inv_cov_g = gp.apply_inverse(np.eye(N))
    #cov = gp.get_matrix(wav_spectro)
    print('george perf = ', time.perf_counter()-t_g)



    ## CELERITE2
    from celerite2 import GP as GP_celerite
    from celerite2 import terms
    # Construct approximate RBF kernel
    t_c = time.perf_counter()
    n_terms = 5
    mus = np.linspace(0.1, 2.0, n_terms) / l
    kernel = 0
    for mu in mus:
        # Quality factor Q=1/sqrt(2) approximates RBF shape
        kernel += terms.SHOTerm(log_S0=np.log(a**2 / n_terms), 
                                log_Q=np.log(1/np.sqrt(2)), 
                                log_omega0=np.log(mu))

    gp = GP_celerite(kernel)
    gp.compute(wav_spectro, err_spectro)

    N = len(wav_spectro)
    inv_cov_c = gp.apply_inverse(np.eye(N))
    print('celerite perf = ', time.perf_counter()-t_c)

    ### Optional: Compare accuracy ###
    diff_norm = np.linalg.norm(inv_cov_g - inv_cov_c)
    print(f"||Δ(inv_cov)|| = {diff_norm:.2e}")
    exit()
    ...
    fig, ax = plt.subplots()
    img = ax.imshow(cov, extent=[wav_spectro[0], wav_spectro[-1], wav_spectro[-1], wav_spectro[0]],
                    norm=SymLogNorm(10**-34, vmin=np.min(cov), vmax=np.max(cov), linscale=5, base=10), cmap='PuBu_r')
    cb=fig.colorbar(img)
    cb.set_label(f'Covariances (W m-2 µm-1)²')
    ax.set_xlabel('wavelength (μm)')
    ax.set_ylabel('wavelength (μm)')
    ax.set_title(f'Flamingos2 covariance for a={a:.2e} W.m⁻².µm⁻¹ and l={l:.2e}µm')
    plt.savefig('test_cov_matrix_Flamingos2.png',  bbox_inches='tight')
    plt.show()
    exit()
