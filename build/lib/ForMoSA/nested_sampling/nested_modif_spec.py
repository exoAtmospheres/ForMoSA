...
def modif_spec(global_params, theta, theta_index,
    ...
    if len(global_params.log_amp) > 3 and len(global_params.log_len) > 3: # If you want separate covariance matrix for each observations
        if global_params.log_amp[indobs*3] != "NA" and global_params.log_len[indobs*3] != "NA":
            ...
            log_amp_picked += np.log(np.median(obs_dict['err_spectro']))
            obs_dict['inv_cov'] = GP_cov_fct(obs_dict['wav_spectro'], obs_dict['err_spectro'], log_amp_picked, log_len_picked)
    ...
    # Calculation of the flux scaling factor phi_bar
        ...
        if global_params.r[0] != "NA" and global_params.d[0] != "NA":
            ...
            if len(global_params.alpha) > 3: # If you want separate alpha for each observations
                ...
                # - - - - - - 
                    ...
                    flx_mod_spectro, flx_mod_photo, scale_spectro, scale_photo = calc_flx_scale(obs_dict, flx_mod_spectro, flx_mod_photo, 0, 0, alpha=0, analytic='yes', mode='analytic', use_cov=use_cov)
