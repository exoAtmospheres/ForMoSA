...
def loglike(theta, theta_index, global_params, main_file, for_plot='no'):
    ...
    for indobs, obs in enumerate(sorted(glob.glob(main_obs_path))):
        ...
        # print(1/np.median(obs_dict_modif['err_spectro'])**2)
        # print(1/obs_dict_modif['err_spectro']**2)
        # print(obs_dict_modif['inv_cov'][:3,:3])
        # exit()
        ...
        if len(obs_dict_modif['wav_spectro']) != 0:
            if global_params.logL_type[indobs % len(global_params.logL_type)] == 'chi2':
                logL_spectro = logL_chi2(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['err_spectro'], full=full)
            elif global_params.logL_type[indobs % len(global_params.logL_type)] == 'chi2_covariance' and len(obs_dict_modif['inv_cov']) != 0:
                logL_spectro = logL_chi2_covariance(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['inv_cov'], full=full)
                ...
                logL_spectro = logL_chi2_noisescaling(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['err_spectro'], full=full)
                ...
                logL_spectro = logL_chi2_noisescaling_covariance(obs_dict_modif['flx_spectro']-flx_mod_spectro_modif, obs_dict_modif['inv_cov'], full=full)
            else:
                print()
